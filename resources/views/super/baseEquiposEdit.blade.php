...
@section('contenido')
    <div class="md:flex md:justify-center">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
            <form action="{{route('super.baseEquipos.update',$data)}}" method="POST">
                @csrf
                @method('PUT')
                
                <div class="mb-5">
                    <label for="codigo" class="mb-2 block uppercase text-gray-500 font-bold">CODIGO</label>
                    <input type="text" id="codigo" name="codigo" placeholder="CODIGO" class="border p-3 w-full rounded-lg @error('codigo') border-red-500 @enderror" value="{{$data->codigo}}">
                    @error('codigo')
                ...
                <div class="mb-5">
                    ...
                    <input type="text" id="serie" name="serie" placeholder="SERIE" class="border p-3 w-full rounded-lg @error('serie') border-red-500 @enderror" value="{{$data->serie}}">
                    @error('serie')
                        <p class="bg-red-400 text-white my-2 rounded-lg text-sm p-2 text-center">{{$message}}</p>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="number" id="valor" name="valor" placeholder="VALOR" class="border p-3 w-full rounded-lg @error('valor') border-red-500 @enderror" value="{{$data->valor}}">
                    @error('valor')
                        <p class="bg-red-400 text-white my-2 rounded-lg text-sm p-2 text-center">{{$message}}</p>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <input type="date" id="fecha" name="fecha" placeholder="FECHA" class="border p-3 w-full rounded-lg @error('fecha') border-red-500 @enderror" value="{{$data->fecha_asignacion}}">
                    @error('fecha')
                        <p class="bg-red-400 text-white my-2 rounded-lg text-sm p-2 text-center">{{$message}}</p>
                    @enderror
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select id="estatus" name="estatus" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach ($estatus as $dat )
                            <option @if($data->estatus_id == $dat->id) selected @endif value="{{$dat->id}}">{{$dat->estatus}}</option>    
                        @endforeach
                ...
                <div class="mb-5">
                    ...
                    <select id="responsable" name="responsable" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        @foreach ($responsable as $dat )
                            <option @if($data->responsable_id == $dat->id) selected @endif value="{{$dat->id}}">{{$dat->clave}} - {{$dat->name}} {{$dat->apellidos}}</option>    
                        @endforeach
                ...
                <div class="mb-5">
                    ...
                    <select id="linea" name="linea" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach ($linea as $dat )
                            <option @if($data->linea_id == $dat->id) selected @endif value="{{$dat->id}}">{{$dat->nombre}}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-5">
                    ...
                    <select id="distrito" name="distrito" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        @foreach ($distrito as $dat )
                            <option @if($data->distrito_id == $dat->id) selected @endif value="{{$dat->id}}">{{$dat->distrito}}</option>    
                        @endforeach
                        
                    </select>
                </div>

                <input type="submit" value="ACTUALIZAR EQUIPO" class="bg-blue-600 hover:bg-blue-800 transition-colors cursor-pointer uppercase font-bold w-full p-3 text-white rounded-lg">
